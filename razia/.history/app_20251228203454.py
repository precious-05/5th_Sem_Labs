...
# =====================================================
# SAMPLE BARCODES (for user guidance)
# =====================================================
sample_barcodes = [
    "737628064502",  # Sample Product 1
    "5410188006706",  # Sample Product 2
    "3017620422003",  # Sample Product 3
    "5000159484695",  # Sample Product 4
    "7622300729280",  # Sample Product 5
]
...
def load_product():
    ...
    if product:
        ...
        # Update global dataframe for CSV
        global current_df
        current_df = df
...
def fill_from_sample(value):
    barcode_input.set_value(value)
...
def main():
    ...
    # Input Card
    with ui.card().classes("w-full p-6 shadow-lg bg-gradient-to-r from-green-100 via-yellow-100 to-pink-100 rounded-xl"):
        ui.label("Enter Barcode or select a sample:").classes("font-semibold")
        ui.label("Example barcodes provided for testing").classes("text-xs text-gray-400")

        with ui.row():
            global barcode_input
            barcode_input = ui.input(placeholder="e.g. 737628064502").classes("w-64")
            
            # Sample barcode dropdown
            sample_select = ui.select(
                sample_barcodes, placeholder="Select sample barcode"
            )
            sample_select.on("change", lambda e: fill_from_sample(e.value))
