...
// 100% CORRECTED VERSION FOR MONGODB BACKEND
// ============================================
...
function getFormData() {
    ...
    try {
        ...
        const TSH_Level = document.getElementById('TSH_Level');
        const T3_Level = document.getElementById('T3_Level');
        const T4_Level = document.getElementById('T4_Level');
        const Nodule_Size = document.getElementById('Nodule_Size');
        const Thyroid_Cancer_Risk = document.getElementById('Thyroid_Cancer_Risk');
        const Gender_Male = document.getElementById('Gender_Male');
        
        // Debug log
        console.log('Form elements found:', {
            Age: Age ? 'Yes' : 'No',
            TSH_Level: TSH_Level ? 'Yes' : 'No',
            T3_Level: T3_Level ? 'Yes' : 'No',
            T4_Level: T4_Level ? 'Yes' : 'No',
            Nodule_Size: Nodule_Size ? 'Yes' : 'No',
            Thyroid_Cancer_Risk: Thyroid_Cancer_Risk ? 'Yes' : 'No',
            Gender_Male: Gender_Male ? 'Yes' : 'No'
        });
        
        const data = {
            Age: Age ? parseFloat(Age.value) || 0 : 0,
            TSH_Level: TSH_Level ? parseFloat(TSH_Level.value) || 0 : 0,
            T3_Level: T3_Level ? parseFloat(T3_Level.value) || 0 : 0,
            T4_Level: T4_Level ? parseFloat(T4_Level.value) || 0 : 0,
            Nodule_Size: Nodule_Size ? parseFloat(Nodule_Size.value) || 0 : 0,
            Thyroid_Cancer_Risk: Thyroid_Cancer_Risk ? parseInt(Thyroid_Cancer_Risk.value) || 0 : 0,
            Gender_Male: Gender_Male ? parseInt(Gender_Male.value) || 0 : 0,
...
// Handle Prediction Submit - 100% WORKING
async function handlePredictionSubmit(e) {
    ...
    try {
        ...
        // Show error message
        showErrorMessage(
            `Failed to get prediction.<br>Error: ${error.message}<br><br>Trying demo mode...`, 
            'Analysis Failed'
        );
...
function createDefaultChart() {
    ...
    if (window.Chart) {
        ...
        // Sample data
        const labels = ['Age', 'TSH', 'T3', 'T4', 'Nodule Size', 'Risk Score'];
        const values = [
            (currentFormData?.Age || 0) / 1.2,
            (currentFormData?.TSH_Level || 0) * 2,
            (currentFormData?.T3_Level || 0) * 3,
            (currentFormData?.T4_Level || 0) / 1.5,
            (currentFormData?.Nodule_Size || 0) * 4,
            (currentFormData?.Thyroid_Cancer_Risk || 0) * 5
        ];
...
function resetForm() {
    ...
    showSuccessMessage('Form has been reset', 'Form Reset');
}
...
function showMockResults() {
    ...
    showInfoMessage('Showing demonstration results (backend not connected)', 'Demo Mode');
}

// Notification Functions (if ThyroScan object doesn't exist)
function showSuccessMessage(message, title = 'Success') {
    if (window.ThyroScan && ThyroScan.showSuccess) {
        ThyroScan.showSuccess(message, title);
    } else {
        alert(`✅ ${title}\n\n${message}`);
    }
}
...
function showErrorMessage(message, title = 'Error') {
    if (window.ThyroScan && ThyroScan.showError) {
        ThyroScan.showError(message, title);
    } else {
        alert(`❌ ${title}\n\n${message}`);
    }
}
...
function showInfoMessage(message, title = 'Info') {
    if (window.ThyroScan && ThyroScan.showInfo) {
        ThyroScan.showInfo(message, title);
    } else {
        alert(` ${title}\n\n${message}`);
    }
}
