    ...
    with ui.card().classes('header-card'):
        ...
        with ui.row().classes('gap-2 mt-4'):
            ui.button('Reset System', 
                     on_click=lambda: simulator.initialize_system()).props('flat color=white')
            ui.button('Help', 
        ...
        with ui.column().classes('left-panel'):
            ...
            with ui.card().classes('card'):
                ...
                with ui.row().classes('items-center mb-4'):
                    ...
                    p_slider = ui.slider(min=2, max=6, value=4)
                    p_label = ui.label('4').classes('ml-2 font-bold')
                
                with ui.row().classes('items-center mb-4'):
                    ui.label('Resources:')
                    r_slider = ui.slider(min=2, max=4, value=3)
                    r_label = ui.label('3').classes('ml-2 font-bold')
            ...
            with ui.card().classes('card'):
                ...
                for i, res in enumerate(simulator.resources):
                    with ui.row().classes('items-center mb-2'):
                        ui.label(f'{res}:')
                        request_inputs.append(ui.number(value=0, min=0, max=3).classes('ml-2'))
                ...
                with panels:
                    ...
                    with ui.tab_panel(tab1):
                        allocated_table = ui.table({
                            'columnDefs': [{'headerName': 'Application', 'field': 'process'}] + 
                                         [{'headerName': res, 'field': f'res_{i}'} 
                                          for i, res in enumerate(simulator.resources)],
                            'rowData': []
                        }).classes('w-full h-64')
                    ...
                    with ui.tab_panel(tab5):
                        history_area = ui.textarea(label='System Events').props('readonly').classes('w-full h-64')
...
def refresh_ui():
    ...
    for i, proc in enumerate(simulator.processes):
        ...
        for j, res in enumerate(simulator.resources):
            row[f'res_{j}'] = int(simulator.allocated[i][j])
        alloc_data.append(row)
    allocated_table.options['rowData'] = alloc_data
    ...
    for i, proc in enumerate(simulator.processes):
        row = {'process': proc}
        for j, res in enumerate(simulator.resources):
            row[f'res_{j}'] = int(simulator.max_need[i][j])
        max_data.append(row)
    ...
    for i, res in enumerate(simulator.resources):
        available_data.append({
            'resource': res,
            'available': int(simulator.available[i])
        })
