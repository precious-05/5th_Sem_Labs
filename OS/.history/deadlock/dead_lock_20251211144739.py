...
class DeadlockSimulator:
    ...
    def initialize_system(self, num_processes: int, num_resources: int):
        """Initialize the system with given number of processes and resources"""
        self.processes = [f"P{i}" for i in range(num_processes)]
        self.resources = [f"R{i}" for i in range(num_resources)]
        
        # Initialize matrices with zeros
        self.allocated = [[0 for _ in range(num_resources)] for _ in range(num_processes)]
        self.max_need = [[0 for _ in range(num_resources)] for _ in range(num_processes)]
        self.request = [[0 for _ in range(num_resources)] for _ in range(num_processes)]
        self.available = [0 for _ in range(num_resources)]
        ...
        for j in range(num_resources):
            total_allocated = sum(self.allocated[i][j] for i in range(num_processes))
            self.available[j] = total_allocated + np.random.randint(1, 5)
        
        self.safe_sequence = []
        self.history.append(f"System initialized with {num_processes} processes and {num_resources} resources")
...
simulator.initialize_system(3, 3)
...
with ui.header(elevated=True).style('background-color: #3B82F6').classes('items-center justify-between'):
    ...
    with ui.row():
        ui.button('Reset System', on_click=lambda: simulator.initialize_system(3, 3)).props('flat color=white')
    ...
    with ui.column().classes('w-1/3 p-4'):
        ...
        with ui.card().tight():
            with ui.card_section():
                ...
                with ui.row().classes('items-center'):
                    ui.label('Processes:')
                    process_slider = ui.slider(min=2, max=5, value=3).bind_value_to(simulator, 'num_processes')
                    ui.label().bind_text_from(process_slider, 'value')
                
                with ui.row().classes('items-center'):
                    ui.label('Resources:')
                    resource_slider = ui.slider(min=2, max=5, value=3).bind_value_to(simulator, 'num_resources')
                    ui.label().bind_text_from(resource_slider, 'value')
                
                ui.button('Apply Configuration', on_click=lambda: simulator.initialize_system(
                    int(process_slider.value), int(resource_slider.value)
                )).props('flat')
        ...
        with ui.card().tight():
            with ui.card_section():
                ...
                def make_request():
                    process_idx = int(process_select.value[1:]) if process_select.value else 0
        ...
        with ui.card().tight():
            with ui.card_section():
                ...
                deadlocked_list = ui.label('Detecting...')
                ...
                def recover_deadlock():
                    process_idx = int(recovery_select.value[1:]) if recovery_select.value else 0
    ...
    with ui.column().classes('w-2/3 p-4'):
        ...
        with ui.tab_panels(matrices_tabs, value='Allocated Matrix').classes('w-full'):
            ...
            with ui.tab_panel('System History'):
                history_text = ui.textarea(label='System Events').classes('w-full h-64')
                ...
                with ui.row().classes('justify-center items-center'):
                    with ui.column().classes('items-center'):
                        ...
                        for i, p in enumerate(simulator.processes):
                            ui.label(p).style(f'background-color: {"#EF4444" if i in simulator.detect_deadlock() else "#10B981"}; color: white; padding: 8px; border-radius: 50%; width: 40px; height: 40px; display: flex; align-items: center; justify-content: center')
                    ...
                    with ui.column().classes('items-center'):
                        ...
                        for i, r in enumerate(simulator.resources):
                            ui.label(r).style('background-color: #3B82F6; color: white; padding: 8px; border-radius: 4px; width: 40px; height: 40px; display: flex; align-items: center; justify-content: center')
...
ui.run(title='Deadlock Simulator', dark=False, reload=False)