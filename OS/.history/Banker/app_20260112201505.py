...
with col1:
    ...
    if st.session_state.system_initialized:
        ...
        <div class="tech-card">
            <div class="tech-card-title">
                <i class="fas fa-microchip"></i> Resource Monitor
            </div>
        """, unsafe_allow_html=True)
...
with col2:
    if st.session_state.system_initialized:
        ...
        <div class="tech-card">
            <div class="tech-card-title">
                <i class="fas fa-table"></i> Resource Allocation Matrix
            </div>
        """, unsafe_allow_html=True)
        ...
        <div class="tech-card">
            <div class="tech-card-title">
                <i class="fas fa-exchange-alt"></i> Resource Request Manager
            </div>
        """, unsafe_allow_html=True)
        ...
        if st.button("📤 Submit Resource Request", use_container_width=True, key="submit_request"):
            if sum(request) > 0:
                ...
                st.warning("⚠️ Please request at least one resource unit")
...
with col3:
    if st.session_state.system_initialized:
        ...
        <div class="tech-card">
            <div class="tech-card-title">
                <i class="fas fa-shield-alt"></i> System Safety Status
            </div>
        """, unsafe_allow_html=True)
        ...
        if is_safe:
            ...
            <div class="status-indicator status-safe">
                <i class="fas fa-check-circle"></i> SYSTEM SAFE
            </div>
            """, unsafe_allow_html=True)
            ...
            <div style="margin-top: 20px; padding: 15px; background: rgba(16, 185, 129, 0.1); 
                ...
                <div style="color: var(--success-light); font-weight: 600; margin-bottom: 10px;">
                    <i class="fas fa-list-ol"></i> Safe Execution Sequence:
                </div>
                <div style="font-family: 'Courier New', monospace; color: var(--text-primary); 
                          padding: 10px; background: rgba(0, 0, 0, 0.2); border-radius: 4px;">
                    {' → '.join([f'{PROCESS_NAMES[p % len(PROCESS_NAMES)]}' for p in sequence])}
                </div>
            </div>
            """, unsafe_allow_html=True)
        else:
            ...
            <div class="status-indicator status-unsafe">
                <i class="fas fa-exclamation-triangle"></i> SYSTEM UNSAFE
            </div>
            """, unsafe_allow_html=True)
            ...
            <div style="margin-top: 20px; padding: 15px; background: rgba(239, 68, 68, 0.1); 
                ...
                <div style="color: var(--danger-light); font-weight: 600; margin-bottom: 10px;">
                    <i class="fas fa-skull-crossbones"></i> Warning: Potential Deadlock
                </div>
        ...
        if st.session_state.request_history:
            ...
            for req in reversed(st.session_state.request_history[-3:]):
                border_color = "var(--success)" if req['granted'] else "var(--danger)"
                icon = "✅" if req['granted'] else "❌"
                
                st.markdown(f"""
                <div style="border-left: 3px solid {border_color}; padding: 12px; margin-bottom: 10px; 
                          background: rgba(30, 41, 59, 0.5); border-radius: 0 8px 8px 0;">
                    <div style="display: flex; justify-content: space-between; align-items: start;">
                        <div>
                            ...
                            <div style="color: var(--text-secondary); font-size: 12px; margin-top: 4px;">
                                <i class="far fa-clock"></i> {req['time']}
                            </div>
                        </div>
                    ...
                    <div style="color: var(--accent-blue-light); font-size: 13px; margin-top: 8px;">
                        Request: {req['request']}
                    </div>
                </div>
                """, unsafe_allow_html=True)
...
<div class="tech-footer">
    <div style="margin-bottom: 15px; font-size: 16px; color: var(--accent-blue); font-weight: 500;">
        <i class="fas fa-laptop-code"></i> OS Deadlock Simulation System
    </div>
    <div style="color: var(--text-primary); margin-bottom: 8px; font-weight: 500;">
...
if not st.session_state.system_initialized:
    ...
    with col2:
        ...
        <div style="text-align: center; padding: 60px 40px; background: rgba(30, 41, 59, 0.8); 
            ...
            <div style="font-size: 72px; color: var(--accent-purple); margin-bottom: 20px;">
                ⚙️
            </div>
            <h3 style="color: var(--accent-blue); margin-bottom: 15px; font-weight: 600;">
            ...
            <div style="color: var(--accent-purple-light); font-size: 14px; padding: 12px; 
                      background: rgba(168, 85, 247, 0.1); border-radius: 8px;">
                <i class="fas fa-info-circle"></i> Set processes & resources, then click "Initialize System"
            </div>
        </div>
        """, unsafe_allow_html=True)