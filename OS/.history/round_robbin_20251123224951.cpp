    ...
    void runScheduler() {
        ...
        while (!readyQueue.empty()) {
            ...
            if (p.remainingTime > timeQuantum)    // 5 > 2 → TRUE
            {
                ...
                // remainingTime → remaining work inside process
                // currentTime →  Time spent on CPU clock

                readyQueue.push(index);   // pushing the process back to the end of queue
            } 


            // timeQuantum = 2

            // p.remainingTime = 1 (less than or equal to time quantum)

            // p.burstTime = 3

            // currentTime = 8 (CPU time before this process runs)

            else 
            {
                currentTime += p.remainingTime;    
                p.waitingTime = currentTime - p.burstTime;
                p.remainingTime = 0;   // process finished
            }
