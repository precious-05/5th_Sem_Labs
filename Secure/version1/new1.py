...
ULTRA MODERN PROFESSIONAL UI with REAL-TIME ANALYTICS DASHBOARD
FUNCTIONALITY: 100% preserved + Real-Time Features Added
"""
...
COLORS = {
    "primary": "#7C3AED",        # Perfect!
    "primary_light": "#F5B5FD",  # Perfect!
    "primary_dark": "#5B21B6",   # Perfect!
    
    "accent": "#38BDF8",         # Great choice
    "accent_pink": "#F9A8D4",    # Maybe #F472B6 for more visibility?
    
    "success": "#10B981",        # Slightly softer green
    "warning": "#F59E0B",        # Perfect
    "danger": "#EF4444",         # Perfect
    
    "dark": "#1F2937",           # Perfect
    "light": "#D8F6C8",          # Love this!
    "surface": "#FFFFFF",        # Perfect
    
    "gray": "#9CA3AF",           # Perfect
    "gray_light": "#F3F4F6",     # Perfect
    "border": "#E5E7EB",         # Perfect
    
    # Optional additions:
    "info": "#38BDF8",           # For info messages
...
class RealTimeWebSocketManager:
    ...
    def _process_message(self, data):
        ...
        elif message_type == 'events_update':
            st.session_state.realtime_events = data.get('data', [])
            
        elif message_type == 'search_completed':
            # Trigger a rerun if we're on the dashboard tab
            st.rerun()
...
# ================================
# HELPER FUNCTIONS - 100% SAME FUNCTIONALITY + REAL-TIME
# ================================
...
def load_dashboard_data():
    ...
    try:
        ...
        events_response = requests.get(f"{BACKEND_URL}/api/realtime-events?limit=10")
        if events_response.status_code == 200:
            st.session_state.dashboard_data['realtime_events'] = events_response.json().get('events', [])
...
def create_realtime_metrics_display():
    ...
    with col1:
        ...
        <div class='metric-card'>
            <div style='font-size: 1rem; color: {COLORS["gray"]}; margin-bottom: 12px; display: flex; align-items: center; justify-content: center; gap: 10px;'>
                <img src='{get_icon_svg("database", COLORS["primary"], 24)}' style='width: 24px; height: 24px;'>
                <span>Total Drugs</span>
            </div>
            <div style='font-size: 2.8rem; font-weight: 800; color: {COLORS["primary"]}; margin-bottom: 8px;'>{metrics.get('total_drugs', 0)}</div>
            <div style='font-size: 0.85rem; color: {COLORS["gray"]}; opacity: 0.8;'>Medications in database</div>
        </div>
    ...
    with col2:
        ...
        <div class='metric-card'>
            <div style='font-size: 1rem; color: {COLORS["gray"]}; margin-bottom: 12px; display: flex; align-items: center; justify-content: center; gap: 10px;'>
                <img src='{get_icon_svg("alert", "#b5179e", 24)}' style='width: 24px; height: 24px;'>
                <span>Critical Pairs</span>
            </div>
            <div style='font-size: 2.8rem; font-weight: 800; color: #b5179e; margin-bottom: 8px;'>{metrics.get('critical_risk_pairs', 0)}</div>
            <div style='font-size: 0.85rem; color: {COLORS["gray"]}; opacity: 0.8;'>Extreme risk pairs</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col3:
        ...
        <div class='metric-card'>
            <div style='font-size: 1rem; color: {COLORS["gray"]}; margin-bottom: 12px; display: flex; align-items: center; justify-content: center; gap: 10px;'>
                <img src='{get_icon_svg("chart-line", COLORS["accent"], 24)}' style='width: 24px; height: 24px;'>
                <span>Avg Risk</span>
            </div>
            <div style='font-size: 2.8rem; font-weight: 800; color: {COLORS["accent"]}; margin-bottom: 8px;'>{metrics.get('avg_risk_score', 0):.1f}%</div>
            <div style='font-size: 0.85rem; color: {COLORS["gray"]}; opacity: 0.8;'>Average confusion risk</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col4:
        ...
        <div class='metric-card'>
            <div style='font-size: 1rem; color: {COLORS["gray"]}; margin-bottom: 12px; display: flex; align-items: center; justify-content: center; gap: 10px;'>
                <img src='{get_icon_svg("users", COLORS["success"], 24)}' style='width: 24px; height: 24px;'>
                <span>Connected</span>
            </div>
            <div style='font-size: 2.8rem; font-weight: 800; color: {COLORS["success"]}; margin-bottom: 8px;'>{metrics.get('connected_clients', 0)}</div>
            <div style='font-size: 0.85rem; color: {COLORS["gray"]}; opacity: 0.8;'>Live dashboard users</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    status_text = {
        ...
        'error': '🔴 System Error'
    }.get(system_status, '⚪️ System Unknown')
    
    st.markdown(f"""
    <div style='margin-top: 1rem; padding: 1rem; background: {status_color}15; border-radius: 15px; 
         border: 2px solid {status_color}30; display: flex; align-items: center; gap: 15px;'>
        <div style='font-size: 1.5rem;'>{status_text.split()[0]}</div>
        <div style='flex: 1;'>
            <div style='font-weight: 600; color: {COLORS["dark"]};'>{status_text}</div>
            <div style='font-size: 0.9rem; color: {COLORS["gray"]}; margin-top: 5px;'>
                Last updated: {st.session_state.get('last_update_time', 'Never')}
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
<style>
    ...
    * {{
    ...
    @keyframes borderGlow {{
        0%, 100% {{ border-color: {COLORS["primary"]}; box-shadow: 0 0 15px rgba(124, 58, 237, 0.3); }}
        50% {{ border-color: {COLORS["accent"]}; box-shadow: 0 0 25px rgba(56, 189, 248, 0.5); }}
    }}
    ...
    .main-header {{
        ...
        letter-spacing: -1px;
        text-shadow: 0 4px 20px rgba(124, 58, 237, 0.1);
    ...
    /* Primary Metric Cards - Visible Gradient BEFORE Hover */
    .metric-card {{
        background: linear-gradient(135deg, 
            rgba(124, 58, 237, 0.1) 0%, 
            rgba(56, 189, 248, 0.08) 50%,
            rgba(201, 138, 255, 0.05) 100%);
        padding: 2rem;
        border-radius: 20px;
        box-shadow: 
    ...
    /* ====================== */
    /* RISK CARDS - Colorful BEFORE Hover */
    /* ====================== */
    ...
    .risk-card.low:hover {{
        background: linear-gradient(135deg, 
            ...
            rgba(209, 250, 229, 0.15) 50%,
            rgba(255, 255, 255, 0.95) 100%);
        border-color: {COLORS["success"]};
    }}
    
    /* ====================== */
    /* MODERN BUTTON SYSTEM - Visible BEFORE Hover */
    /* ====================== */
    ...
    /* Secondary Button - Visible BEFORE Hover */
    .stButton > button[kind="secondary"] {{
        background: linear-gradient(135deg, 
            rgba(124, 58, 237, 0.1) 0%, 
            rgba(196, 181, 253, 0.08) 100%) !important;
    ...
    .stButton > button[kind="success"] {{
        background: linear-gradient(135deg, {COLORS["success"]} 0%, #0D9488 100%) !important;
    }}
    ...
    .stButton > button[kind="warning"] {{
        background: linear-gradient(135deg, {COLORS["warning"]} 0%, #D97706 100%) !important;
    }}
    ...
    /* ====================== */
    /* MODERN TABS - Colorful BEFORE Hover */
    /* ====================== */
    ...
    /* ====================== */
    /* MODERN BADGES - Colorful BEFORE Hover */
    /* ====================== */
    ...
    /* ====================== */
    /* GRID ITEMS - Colorful BEFORE Hover */
    /* ====================== */
    ...
    /* ====================== */
    /* STATUS INDICATORS - Colorful BEFORE Hover */
    /* ====================== */
    ...
    ::-webkit-scrollbar-thumb:hover {{
        background: linear-gradient(135deg, {COLORS["primary_dark"]} 0%, #4C1D95 100%);
    }}
...
# Sub-header with real-time indicator
if st.session_state.get('websocket_connected'):
    connection_status = f"""
    <span class='realtime-connection connection-connected'>
    ...
    connection_status = """
    <span class='realtime-connection connection-disconnected'>
        OFFLINE
    </span>
    """

st.markdown(f"""
<div class='sub-header'>AI-Powered Medication Safety Intelligence Platform {connection_status}</div>
""", unsafe_allow_html=True)
    ...
    if st.button(f"**Quick Demo**", use_container_width=True):
        ...
        with st.spinner("Testing with Metformin..."):
            ...
            if result:
                ...
                st.rerun()
with action_cols[3]:
    if st.button(f"**Connect Live**", use_container_width=True, type="secondary"):
...
# ================================
# TAB 2: ANALYTICS DASHBOARD (UNCHANGED)
# ================================
...
# ================================
# TAB 3: REAL-TIME DASHBOARD (NEW FEATURE)
# ================================
...
with tab3:
    ...
    st.markdown(f"""
    <div style='text-align: center; margin-bottom: 3rem;'>
        <img src='{get_icon_svg("bolt", COLORS["accent"], 80)}' style='margin-bottom: 20px; display: block; animation: pulse 1s infinite;'>
        <h2 class='section-header'>Live Real-Time Dashboard</h2>
        <p style='color: {COLORS["gray"]}; font-size: 1.2rem; max-width: 600px; margin: 0 auto; line-height: 1.6;'>
            Monitor medication safety metrics in real-time with live WebSocket updates every 5 seconds
        </p>
    </div>
    """, unsafe_allow_html=True)
    
    # Connection Status
    connection_status_cols = st.columns([2, 1])
    with connection_status_cols[0]:
        if st.session_state.get('websocket_connected'):
            st.markdown("""
            <div style='padding: 1rem; background: rgba(16, 185, 129, 0.1); border-radius: 15px; 
                 border: 2px solid rgba(16, 185, 129, 0.3);'>
                <div style='display: flex; align-items: center; gap: 15px;'>
                    <div style='width: 12px; height: 12px; background-color: #10B981; border-radius: 50%; 
                         animation: pulse 1s infinite; box-shadow: 0 0 10px rgba(16, 185, 129, 0.5);'></div>
                    <div>
                        <div style='font-weight: 700; color: #10B981;'>CONNECTED TO LIVE DATA STREAM</div>
                        <div style='font-size: 0.9rem; color: #6B7280; margin-top: 5px;'>
                            Receiving real-time updates from backend server
                        </div>
                    </div>
                </div>
            </div>
            ...
            st.markdown("""
            <div style='padding: 1rem; background: rgba(239, 71, 111, 0.1); border-radius: 15px; 
                 border: 2px solid rgba(239, 71, 111, 0.3);'>
                <div style='display: flex; align-items: center; gap: 15px;'>
                    <div style='width: 12px; height: 12px; background-color: #EF4444; border-radius: 50%;'></div>
                    <div>
                        <div style='font-weight: 700; color: #EF4444;'>REAL-TIME CONNECTION OFFLINE</div>
                        <div style='font-size: 0.9rem; color: #6B7280; margin-top: 5px;'>
                            Click "Connect Live" button to enable real-time updates
                        </div>
                    </div>
                </div>
            </div>
    ...
    with connection_status_cols[1]:
        if st.button(f"**{'Refresh Connection' if st.session_state.get('websocket_connected') else 'Connect Live'}**", 
                    use_container_width=True, type="primary"):
            websocket_manager.start_connection()
            if not st.session_state.get('websocket_connected'):
                st.success("Starting real-time connection...")
            st.rerun()
    ...
    st.markdown(f"""
    <h3 style='color: {COLORS["dark"]}; margin-bottom: 2rem; display: flex; align-items: center; gap: 15px;'>
        <img src='{get_icon_svg("tachometer", COLORS["primary"], 24)}' style='width: 24px; height: 24px;'>
        Live System Metrics
    </h3>
    """, unsafe_allow_html=True)
    
    # Display real-time metrics
    create_realtime_metrics_display()
    ...
    st.markdown(f"""
    <h3 style='color: {COLORS["dark"]}; margin-bottom: 2rem; display: flex; align-items: center; gap: 15px;'>
        <img src='{get_icon_svg("bell", COLORS["primary"], 24)}' style='width: 24px; height: 24px;'>
        Live Events Feed
    </h3>
    """, unsafe_allow_html=True)
    
    # Load recent events if not loaded
    if not st.session_state.realtime_events:
        try:
            response = requests.get(f"{BACKEND_URL}/api/realtime-events?limit=10")
            if response.status_code == 200:
                st.session_state.realtime_events = response.json().get('events', [])
        except:
            pass
    ...
    if st.session_state.realtime_events:
        ...
        with events_container:
            for event in st.session_state.realtime_events[:10]:
                ...
                <div style='margin-bottom: 1rem; padding: 1rem; border-radius: 12px; 
                     ...
                     border-left: 4px solid {color}; border: 1px solid {color}20;'>
                    <div style='display: flex; align-items: flex-start; gap: 15px;'>
                        <div style='margin-top: 3px;'>
                            <img src='{get_icon_svg(icon, color, 20)}' style='width: 20px; height: 20px;'>
                        </div>
                        <div style='flex: 1;'>
                            <div style='font-weight: 600; color: {COLORS["dark"]}; margin-bottom: 5px;'>
                                {event_type.replace('_', ' ').title()}
                            </div>
                            <div style='font-size: 0.9rem; color: {COLORS["gray"]}; margin-bottom: 5px;'>
                                {event_data.get('message', str(event_data))}
                            </div>
                            <div style='font-size: 0.8rem; color: {COLORS["gray"]}; opacity: 0.7;'>
        ...
        st.info("No recent events. Events will appear here when real-time connection is active.")
    
    # Recent Searches
    st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
    st.markdown(f"""
    <h3 style='color: {COLORS["dark"]}; margin-bottom: 2rem; display: flex; align-items: center; gap: 15px;'>
        <img src='{get_icon_svg("search", COLORS["primary"], 24)}' style='width: 24px; height: 24px;'>
        Recent Searches
    </h3>
    """, unsafe_allow_html=True)
    
    if st.session_state.realtime_metrics and 'recent_searches' in st.session_state.realtime_metrics:
        recent_searches = st.session_state.realtime_metrics.get('recent_searches', [])
        
        if recent_searches:
            search_cols = st.columns(2)
            for i, search in enumerate(recent_searches[:6]):
                with search_cols[i % 2]:
                    drug_name = search.get('drug_name', 'Unknown')
                    risks_found = search.get('risks_found', 0)
                    ...
                    <div class='grid-item' style='margin-bottom: 1rem;'>
                        <div style='display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;'>
                            <div style='font-weight: 600; color: {COLORS["dark"]};'>{drug_name}</div>
                            <div style='font-size: 0.9rem; color: {COLORS["gray"]};'>
                                {timestamp.split('T')[1].split('.')[0] if 'T' in timestamp else ''}
                            </div>
                        </div>
                        <div style='display: flex; gap: 20px;'>
                            <div>
                                <div style='font-size: 0.85rem; color: {COLORS["gray"]};'>Risks Found</div>
                                <div style='font-size: 1.2rem; font-weight: 700; color: {COLORS["primary"]};'>{risks_found}</div>
                            </div>
                            <div>
                                <div style='font-size: 0.85rem; color: {COLORS["gray"]};'>Highest Risk</div>
                                <div style='font-size: 1.2rem; font-weight: 700; color: {COLORS["danger"] if highest_risk > 50 else COLORS["warning"]};'>
                                    {highest_risk:.0f}%
                                </div>
                            </div>
                        </div>
            ...
            st.info("No recent searches recorded.")
    else:
        st.info("Search data will appear here when real-time connection is active.")
    
    # Performance Metrics
    st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
    st.markdown(f"""
    <h3 style='color: {COLORS["dark"]}; margin-bottom: 2rem; display: flex; align-items: center; gap: 15px;'>
        <img src='{get_icon_svg("server", COLORS["primary"], 24)}' style='width: 24px; height: 24px;'>
        System Performance
    </h3>
    """, unsafe_allow_html=True)
    ...
    with perf_cols[0]:
        ...
        <div class='metric-card' style='min-height: 140px;'>
            <div style='font-size: 1rem; color: {COLORS["gray"]}; margin-bottom: 12px; display: flex; align-items: center; justify-content: center; gap: 10px;'>
                <img src='{get_icon_svg("clock", COLORS["accent"], 24)}' style='width: 24px; height: 24px;'>
                <span>Update Interval</span>
            </div>
            <div style='font-size: 2rem; font-weight: 800; color: {COLORS["accent"]}; margin-bottom: 8px;'>5s</div>
            <div style='font-size: 0.85rem; color: {COLORS["gray"]}; opacity: 0.8;'>Real-time refresh rate</div>
        </div>
        """, unsafe_allow_html=True)
    
    with perf_cols[1]:
        avg_risk = st.session_state.realtime_metrics.get('avg_risk_score', 0) if st.session_state.realtime_metrics else 0
        st.markdown(f"""
        <div class='metric-card' style='min-height: 140px;'>
            <div style='font-size: 1rem; color: {COLORS["gray"]}; margin-bottom: 12px; display: flex; align-items: center; justify-content: center; gap: 10px;'>
                <img src='{get_icon_svg("chart-line", COLORS["primary"], 24)}' style='width: 24px; height: 24px;'>
                <span>Avg Risk</span>
            </div>
            <div style='font-size: 2rem; font-weight: 800; color: {COLORS["primary"]}; margin-bottom: 8px;'>{avg_risk:.1f}%</div>
            <div style='font-size: 0.85rem; color: {COLORS["gray"]}; opacity: 0.8;'>Current average</div>
        </div>
        """, unsafe_allow_html=True)
    
    with perf_cols[2]:
        clients = st.session_state.realtime_metrics.get('connected_clients', 0) if st.session_state.realtime_metrics else 0
        st.markdown(f"""
        <div class='metric-card' style='min-height: 140px;'>
            <div style='font-size: 1rem; color: {COLORS["gray"]}; margin-bottom: 12px; display: flex; align-items: center; justify-content: center; gap: 10px;'>
                <img src='{get_icon_svg("users", COLORS["success"], 24)}' style='width: 24px; height: 24px;'>
            ...
            <div style='font-size: 2rem; font-weight: 800; color: {COLORS["success"]}; margin-bottom: 8px;'>{clients}</div>
            <div style='font-size: 0.85rem; color: {COLORS["gray"]}; opacity: 0.8;'>Connected dashboards</div>
        </div>
        """, unsafe_allow_html=True)

# ================================
# TAB 4: ABOUT & RESOURCES (UNCHANGED)
# ================================
...
# ================================
# AUTO-START WEBSOCKET CONNECTION
# ================================

# Auto-start WebSocket connection when app loads
if not st.session_state.get('websocket_connected'):
    websocket_manager.start_connection()