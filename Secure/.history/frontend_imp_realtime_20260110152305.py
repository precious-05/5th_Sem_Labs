...
def create_heatmap_chart():
    """Create heatmap using Plotly Express (simpler approach)"""
    ...
    # Plotly Express se heatmap banayein
    fig = px.imshow(
        risk_matrix,
        x=drug_names,
        y=drug_names,
        ...
        text_auto=".0f",  # Automatic text formatting
    )
    ...
    fig.update_layout(
        title=dict(
            ...
            font=dict(size=20, family="Poppins", color=COLORS['text_primary']),
            x=0.5
        ),
        height=500,
        xaxis=dict(
        ...
        coloraxis_colorbar=dict(
            title="Risk %",
            title_font=dict(size=12, family="Poppins"),
