...
def create_heatmap_chart():
    """Create interactive drug confusion heatmap with professional annotations"""
    ...
    # Create annotations for all cells (not just high-risk)
    annotations = []
    for i, row in enumerate(risk_matrix_np):
        for j, value in enumerate(row):
            # Determine text color based on background
            if value > 50:  # High values use white text
                text_color = "white"
                font_weight = "bold"
            else:  # Lower values use dark text
                text_color = "#111827"
                font_weight = "normal"
            
            # Only show annotations for values above 10% to avoid clutter
            if value > 10:
                annotations.append(
                    dict(
                        x=j,
    ...
    # Professional color scale matching app theme
    colorscale = [
        [0.0, "#10B981"],   # Primary green for low risk
        [0.25, "#3B82F6"],  # Blue
        [0.5, "#8B5CF6"],   # Purple
        [0.75, "#F59E0B"],  # Warning orange
        [1.0, "#EF4444"]    # Danger red for high risk
    ]
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        zmax=100,
        hoverongaps=False,
        hovertemplate="<b>%{y}</b> ↔ <b>%{x}</b><br>Confusion Risk: <b>%{z:.1f}%</b><extra></extra>",
        colorbar=dict(
            title="Risk Score %",
            titleside="right",
            titlefont=dict(
        ...
        text=risk_matrix_np,  # This enables text display
        ...
        textfont=dict(
            ...
            color=[
                [
                    "white" if val > 50 else "#111827" 
                    for val in row
                ] for row in risk_matrix_np
            ]
        )
    ))
    ...
    fig.update_layout(
        ...
        width=max(600, len(drug_names) * 40),   # Dynamic width
        xaxis=dict(
            title=dict(
                text="Drug Names",
                font=dict(
        ...
        yaxis=dict(
            ...
            linewidth=1,
            showgrid=True,
            autorange="reversed",  # For better heatmap display
            ticks="outside",
            ticklen=5
        ),
        plot_bgcolor='white',
    ...
    fig.update_xaxes(showline=True, linewidth=1, linecolor=COLORS['border'])
    fig.update_yaxes(showline=True, linewidth=1, linecolor=COLORS['border'])
    
    # Add annotations if needed (alternative approach)
    if annotations:
