...
with tab3:
    ...
    if st.session_state.websocket_connected:
        ...
        st.warning("⚠️ Real-time connection not active. Click 'Connect Live' button.")
    ...
    if metrics:
        ...
        # Display metrics using simple function
        ...
        st.info("No real-time data available yet. The dashboard will update automatically when data arrives.")
    
    # Manual metrics fetch
    if st.button("Get Latest Metrics"):
        try:
            response = requests.get(f"{BACKEND_URL}/api/metrics")
            if response.status_code == 200:
                st.session_state.realtime_metrics = response.json()
