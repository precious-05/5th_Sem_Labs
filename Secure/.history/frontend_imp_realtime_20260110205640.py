...
def create_risk_breakdown_chart():
    ...
    fig = go.Figure(data=[go.Pie(
        labels=categories,
        values=counts,
        hole=0.6,  # Larger hole for modern donut
        marker=dict(
        ...
        hovertemplate="<b>%{label}</b><br>Count: %{value}<br>Percentage: %{percent}<br>Risk Level<extra></extra>",
        pull=[0.02, 0.02, 0.02, 0.02],  # Slight pull for 3D effect
        rotation=45
    )])
    ...
    fig.update_layout(
        ...
        height=400,  # Same height as bar chart
        width=400,   # Same width as bar chart
        showlegend=True,
        plot_bgcolor='rgba(245, 243, 255, 0.3)',  # Light purple transparent background
        paper_bgcolor='rgba(255, 255, 255, 0.7)',  # Semi-transparent white
        font_color=COLORS['text_primary'],
        legend=dict(
            font=dict(size=11, color=COLORS['text_secondary'], family="Inter", weight=500),
        ...
        margin=dict(l=20, r=180, t=80, b=40),  # Adjusted for legend
        annotations=[
            dict(
                text=f"<b>Total</b><br>{sum(counts)}",
                x=0.5, y=0.5,
                font=dict(size=20, color=COLORS['text_primary'], family="Inter", weight='bold'),
                showarrow=False,
...
def create_top_risks_chart():
    """Create modern vertical bar chart for top risks"""
    ...
    # Get top 10 pairs (limit if more)
    top_risks = top_risks[:10]
    
    pairs = [f"{item['drug1']} ↔ {item['drug2']}" for item in top_risks]
    scores = [item['risk_score'] for item in top_risks]
    
    # Create gradient colors based on risk score
    colors = []
    for score in scores:
        if score >= 75:
            colors.append('#EF4444')  # Critical - Red
        elif score >= 50:
            colors.append('#F59E0B')  # High - Orange
        elif score >= 25:
            colors.append('#8B5CF6')  # Medium - Purple
        else:
            colors.append('#10B981')  # Low - Green
    
    # Create gradient effects for each bar
    gradient_fills = []
    for color in colors:
        # Create a vertical gradient from dark to light
        if color == '#EF4444':  # Red gradient
            gradient_fills.append(f'linear-gradient(0deg, {color}, rgba(239, 68, 68, 0.7))')
        elif color == '#F59E0B':  # Orange gradient
            gradient_fills.append(f'linear-gradient(0deg, {color}, rgba(245, 158, 11, 0.7))')
        elif color == '#8B5CF6':  # Purple gradient
            gradient_fills.append(f'linear-gradient(0deg, {color}, rgba(139, 92, 246, 0.7))')
        else:  # Green gradient
            gradient_fills.append(f'linear-gradient(0deg, {color}, rgba(16, 185, 129, 0.7))')
    ...
    fig = go.Figure(data=[
        go.Bar(
            ...
            marker=dict(
                ...
                gradient=dict(
                    type='vertical',
                    colorscale=[[0, 'rgba(255,255,255,0.8)'], [1, 'rgba(255,255,255,0.1)']]
                )
            ),
    ...
    fig.update_layout(
        title=dict(
            text="<b>Top 10 High-Risk Drug Pairs</b>",
            font=dict(
                color=COLORS['text_primary'], 
                size=18, 
                family="Inter",
        ...
        yaxis=dict(
            ...
            linewidth=2,
            showgrid=True,
            range=[0, max(scores) * 1.15 if scores else 105],  # Dynamic range
            tickmode='linear',
            dtick=20
        ),
        height=400,  # Same height as pie chart
        width=500,   # Slightly wider for labels
        plot_bgcolor='rgba(245, 243, 255, 0.3)',  # Light purple transparent background
        paper_bgcolor='rgba(255, 255, 255, 0.7)',  # Semi-transparent white
        font_color=COLORS['text_primary'],
    ...
    for i, (pair, score, color) in enumerate(zip(pairs, scores, colors)):
        fig.add_annotation(
            x=i,
            y=score + (max(scores) * 0.02 if scores else 3),
            text=f"{score:.1f}%",
            showarrow=False,
            font=dict(size=10, color=color, family="Inter", weight='bold'),
            bgcolor="rgba(255, 255, 255, 0.9)",
            bordercolor=color,
            borderwidth=1,
            borderpad=3
    ...
    if scores:
        ...
        # Add horizontal line at 75% (critical threshold)
        if max_score >= 75:
            fig.add_hline(
                y=75,
                line_dash="dash",
        ...
        # Add horizontal line at 50% (high threshold)
        if max_score >= 50:
            fig.add_hline(
                y=50,
                line_dash="dot",
...
def render_analytics_tab():
    ...
    # CSS Styles with double curly braces for f-string
    st.markdown(f"""
    <style>
