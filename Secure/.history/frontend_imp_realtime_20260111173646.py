...
try:
    # Read your image and convert to base64
    with open("m11.jpg", "rb") as img_file:
        img_data = img_file.read()
        b64_encoded = base64.b64encode(img_data).decode()
    
    # Create HTML to set favicon
    st.markdown(f"""
    <link rel="icon" href="data:image/jpg;base64,{b64_encoded}" type="image/jpg">
    """, unsafe_allow_html=True)
    
    # Set page config
    st.set_page_config(
        page_title="MediNomix | Medication Safety",
        page_icon="💊",  # Ye temporary rahega, HTML wala override karega
        layout="wide",
        initial_sidebar_state="expanded"
    )
    
except Exception as e:
    # Agar image nahi mili toh default icon use karo
    st.set_page_config(
        page_title="MediNomix | Medication Safety",
        page_icon="💊",
        layout="wide",
