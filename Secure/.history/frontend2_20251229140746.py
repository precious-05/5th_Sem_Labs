...
def render_stat_card(icon, value, label, col):
    """Render a professional statistic card with hover effects"""
    with col:
        st.markdown(f"""
        <div class="professional-stat-card">
            <div class="stat-card-inner">
                ...
                <div class="stat-content">
                    <div class="stat-number" data-value="{value}">0</div>
                    <div class="stat-label">{label}</div>
                </div>
                <div class="stat-trend">
                    <svg class="trend-line" viewBox="0 0 100 40" preserveAspectRatio="none">
                        <path d="M0,35 L25,25 L50,30 L75,20 L100,25" 
                              fill="none" 
        ...
        <script>
        setTimeout(() => {{
            const number = document.querySelector('.stat-card-inner .stat-number');
            const target = {value};
            let current = 0;
...
def render_feature_card(icon, title, description, col):
    """Render a feature card with 3D hover and gradient effects"""
    with col:
        # Generate unique ID for each card
        card_id = f"feature_{hash(title)}"
        
        st.markdown(f"""
        <div class="professional-feature-card" id="{card_id}">
            ...
            <div class="feature-card-shadow"></div>
        </div>
        
        <script>
        document.getElementById('{card_id}').addEventListener('mouseenter', function(e) {{
...
def render_glass_card(title, content=None):
    """Render a glassmorphism card with parallax and blur effects"""
    # Generate unique gradient
    import random
    gradient_id = f"gradient_{random.randint(1000, 9999)}"
    
    st.markdown(f"""
    <div class="professional-glass-card">
        <svg class="glass-bg" width="0" height="0">
            <defs>
                <linearGradient id="{gradient_id}" x1="0%" y1="0%" x2="100%" y2="100%">
                    <stop offset="0%" stop-color="{COLORS['primary']}" stop-opacity="0.2"/>
                    <stop offset="50%" stop-color="{COLORS['purple']}" stop-opacity="0.15"/>
                    <stop offset="100%" stop-color="{COLORS['info']}" stop-opacity="0.1"/>
                </linearGradient>
            </defs>
        </svg>
            ...
            <div class="glass-card-content">
                ...
                <div class="glass-card-footer">
                    <div class="footer-stats">
                        ...
                        <div class="stat-item">
                            <span class="stat-icon">⚡</span>
                            <span class="stat-text">Updated</span>
                        </div>
                    </div>
        ...
        <div class="glass-card-reflection"></div>
    ...
    .glass-card-title {{
        font-size: 22px;
        font-weight: 800;
        background: linear-gradient(135deg, {COLORS['text_primary']} 0%, {COLORS['primary']} 100%);
    ...
    .professional-glass-card:hover .glass-card-reflection {{
        opacity: 1;
    }}
    </style>
    """, unsafe_allow_html=True)        
...
def render_realtime_tab():
    """Render Real-Time Dashboard tab - GUARANTEED TO SHOW DATA"""
    
    render_glass_card(
        "⚡ Real-Time Medication Safety Dashboard",
        "Live monitoring with guaranteed data display"
    )
    
    # FORCE LOAD DATA - Don't wait for WebSocket
    if not st.session_state.realtime_metrics:
        ...
        st.session_state.realtime_metrics = {
            ...
            "avg_risk_score": 45.7,
            "recent_searches": [
                {"drug_name": "Lamictal", "similar_drugs_found": 8, "highest_risk": 92.5},
                {"drug_name": "Metformin", "similar_drugs_found": 6, "highest_risk": 76.8},
                {"drug_name": "Celebrex", "similar_drugs_found": 7, "highest_risk": 88.3},
            ],
            "system_status": "live",
    ...
    with col1:
        if st.session_state.websocket_connected:
            ...
            st.info("🔌 WebSocket connecting in background...")
    ...
    render_stat_card("💊", metrics['total_drugs'], "Total Drugs", col1)
    render_stat_card("🔥", metrics['critical_risk_pairs'], "Critical Pairs", col2)
    render_stat_card("⚠️", metrics['high_risk_pairs'], "High Risk Pairs", col3)
    render_stat_card("📈", f"{metrics['avg_risk_score']:.1f}%", "Avg Risk Score", col4)
    
    # Recent Activity - Always show
    st.markdown("### 🕒 Recent Searches")
    
    for search in metrics['recent_searches']:
        with st.expander(f"💊 {search['drug_name']} - {search['similar_drugs_found']} similar drugs found", expanded=False):
            col1, col2 = st.columns(2)
            with col1:
                st.metric("Highest Risk", f"{search['highest_risk']}%")
            with col2:
                st.metric("Similar Drugs", search['similar_drugs_found'])
            st.info(f"**Status:** Analyzed successfully | **Time:** {metrics['last_updated'][11:19]} UTC")
    ...
    if st.session_state.websocket_connected:
        ...
        **✅ Live Connection Active**
        - Connected clients: {metrics['connected_clients']}
        - Last update: {metrics['last_updated'][11:19]} UTC
        - System: {metrics['system_status'].upper()}
        """)
    else:
        st.info("""
        **🔄 Using Enhanced Demo Mode**
        - Real-time data will appear when connection is established
        - All charts show realistic demo data
        - Search functionality works normally
        """)
    
    # Extra: Show WebSocket status
    with st.expander("🔧 Connection Details"):
        st.code(f"""
        Backend URL: {BACKEND_URL}
        WebSocket URL: {WS_URL}
        Connection Status: {'Connected' if st.session_state.websocket_connected else 'Connecting...'}
        Last Update: {metrics['last_updated']}
        """)
