...
# ============================================
# ENHANCED COLOR SCHEME - Professional Healthcare Analytics
# ============================================
COLORS = {
    ...
    # Surface & background (reduced white dominance)
    "dark": "#1F2937",
    "light": "#F8FAFC",  # Slightly warmer off-white
    "surface": "#F1F5F9",  # Soft gray instead of pure white
    ...
    # ===== NEW: PROFESSIONAL DATA VISUALIZATION PALETTES =====
    # Sequential palette for heatmaps (Purple-based, professional)
    "seq_purple_10": "#F5F3FF",  # Lightest
    "seq_purple_20": "#DDD6FE",
    "seq_purple_30": "#C4B5FD",
    "seq_purple_40": "#A78BFA",
    "seq_purple_50": "#8B5CF6",
    "seq_purple_60": "#7C3AED",
    "seq_purple_70": "#6D28D9",
    "seq_purple_80": "#5B21B6",
    "seq_purple_90": "#4C1D95",
    
    # Diverging palette for risk scores (Blue to Red)
    "div_blue_10": "#E0F2FE",
    "div_blue_30": "#7DD3FC",
    "div_blue_50": "#0EA5E9",
    "div_neutral": "#F1F5F9",  # Middle neutral
    "div_red_50": "#EF4444",
    "div_red_70": "#DC2626",
    "div_red_90": "#991B1B",
    
    # Categorical palette for charts (per IBM Design System)
    "cat_purple": "#7C3AED",
    "cat_cyan": "#38BDF8",
    "cat_teal": "#0D9488",
    "cat_magenta": "#DB2777",
...
def get_icon_svg(icon_name, color="#4361ee", size=24):
    ...
    icon_map = {
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="{size}" height="{size}" viewBox="0 0 24 24" fill="none" stroke="{color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <path d="M12 22V8"/>
            <path d="M2 12h4l2-4 2 4h4l2-4 2 4h4"/>
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="{size}" height="{size}" viewBox="0 0 24 24" fill="none" stroke="{color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <path d="M8 12h8"/>
            <path d="M4 21c1.1 0 1.1-1 2.3-1s1.1 1 2.3 1c1.1 0 1.1-1 2.3-1 1.1 0 1.1 1 2.3 1 1.1 0 1.1-1 2.3-1 1.1 0 1.1 1 2.3 1"/>
        </svg>
        """,
        "thermometer": f"""
        <svg xmlns="http://www.w3.org/2000/svg" width="{size}" height="{size}" viewBox="0 0 24 24" fill="none" stroke="{color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
def create_heatmap_chart():
    """Create professional interactive drug confusion heatmap with enhanced visuals"""
    ...
    colorscale = [
        [0.0, COLORS["seq_purple_10"]],    # Very low risk
        [0.2, COLORS["seq_purple_30"]],    # Low risk
        [0.4, COLORS["seq_purple_50"]],    # Medium risk
        [0.6, COLORS["seq_purple_70"]],    # High risk
        [0.8, "#B45309"],                  # Warning (orange-brown)
        [1.0, COLORS["danger"]]            # Critical risk
    ]
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        zmin=0,
        zmax=100,
        text=[[f"{val:.0f}%" if val > 15 else "" for val in row] for row in risk_matrix],
        texttemplate="<b>%{text}</b>",
        textfont={"size": 10, "color": "white", "family": "'Inter', sans-serif"},
        hoverongaps=False,
        hoverinfo="text",
        hovertemplate="<b>%{y}</b> ↔ <b>%{x}</b><br><b>Risk: %{z:.1f}%</b><extra></extra>",
        showscale=True,
        colorbar=dict(
            title=dict(
                text="<b>Risk Level</b>",
    ...
    fig.update_layout(
        ...
        height=700 if len(drug_names) > 10 else 600,
        margin={"t": 100, "b": 120, "l": 150, "r": 80},
    ...
    fig.add_annotation(
        ...
        text="<span style='background-color:#F5F3FF; padding:5px 10px; border-radius:4px; border:1px solid #DDD6FE;'><b>Low Risk:</b> 0-25%</span>",
        showarrow=False,
    ...
    fig.add_annotation(
        ...
        text="<span style='background-color:#A78BFA; padding:5px 10px; border-radius:4px; border:1px solid #8B5CF6; color:white;'><b>High Risk:</b> 50-75%</span>",
        showarrow=False,
        font=dict(size=10, family="'Inter', sans-serif")
    )
    ...
    fig.add_annotation(
        ...
        text="<span style='background-color:#EF4444; padding:5px 10px; border-radius:4px; border:1px solid #DC2626; color:white;'><b>Critical:</b> 75-100%</span>",
        showarrow=False,
        font=dict(size=10, family="'Inter', sans-serif")
    )
...
def create_risk_breakdown_chart():
    ...
    fig = go.Figure(data=[
        go.Pie(
            ...
            pull=[0.05 if cat == "Critical" else 0 for cat in categories]
        )
    ])
    ...
    fig.update_layout(
        title={
            ...
            "xanchor": "center",
            "yanchor": "top",
            "font": {"size": 20, "color": COLORS["dark"], "family": "'Inter', sans-serif"}
        },
        height=450,
        ...
        plot_bgcolor=COLORS["surface"],
        paper_bgcolor=COLORS["light"],
        font=dict(family="'Inter', sans-serif", size=12, color=COLORS["dark"]),
    )
    ...
    fig.add_annotation(
        ...
        x=0.5, y=0.5,
        font=dict(size=16, color=COLORS["dark"], family="'Inter', sans-serif"),
        showarrow=False
    )
...
def create_top_risks_chart():
    ...
    # Enhanced color mapping with gradient based on score
    colors = []
    for score in scores:
        if score >= 75:
            colors.append(COLORS["danger"])
        elif score >= 50:
            colors.append("#B45309")  # Orange-brown
        elif score >= 25:
    ...
    fig = go.Figure(data=[
        go.Bar(
            ...
            textfont=dict(size=11, color=COLORS["dark"], family="'Inter', sans-serif"),
    ...
    fig.update_layout(
        title={
            ...
            "xanchor": "center",
            "yanchor": "top",
            "font": {"size": 22, "color": COLORS["dark"], "family": "'Inter', sans-serif"}
        },
    ...
    fig.add_vline(x=75, line_dash="dash", line_color=COLORS["danger"], opacity=0.3, 
                  annotation_text="Critical", annotation_position="top right")
    fig.add_vline(x=50, line_dash="dash", line_color="#B45309", opacity=0.3, 
...
<style>
    ...
    * {{
    ...
    .dashboard-card:hover {{
        ...
        box-shadow: 
            0 10px 25px -3px rgba(124, 58, 237, 0.12),
    ...
    /* Risk Cards with Gradient Badges */
    .risk-card {{
        border-left-width: 6px;
    }}
    
    .risk-card.critical {{ border-left-color: {COLORS["danger"]}; }}
    .risk-card.high {{ border-left-color: #B45309; }}
    .risk-card.medium {{ border-left-color: {COLORS["warning"]}; }}
    .risk-card.low {{ border-left-color: {COLORS["success"]}; }}
    ...
    .risk-badge.critical {{
        background: linear-gradient(135deg, rgba(239, 71, 111, 0.15), rgba(219, 39, 119, 0.1));
        color: {COLORS["danger"]};
    ...
    .stTabs [data-baseweb="tab"]:hover {{
        background: linear-gradient(135deg, rgba(124, 58, 237, 0.1) 0%, rgba(91, 33, 182, 0.05) 100%) !important;
        color: {COLORS["primary"]} !important;
...
<div style='position: relative; text-align: center; padding: 3rem 0 2rem 0;'>
    ...
    <div style='position: absolute; top: 100px; right: 10%; width: 150px; height: 150px; 
         background: linear-gradient(135deg, #7C3AED20, #38BDF820); border-radius: 50%; 
    ...
    if st.session_state.search_results:
        ...
        for idx, result in enumerate(filtered_results):
            ...
            with st.container():
                # Card Container
                st.markdown(f"""
                <div class='dashboard-card risk-card {risk_class}'>
                    <div style='display: flex; justify-content: space-between; align-items: start; margin-bottom: 1.5rem;'>
                        ...
                        <div style='text-align: right;'>
                            <span class='risk-badge {risk_class}' style='margin-bottom: 10px; display: inline-block;'>
...
with tab2:
    ...
    if 'metrics' in st.session_state.dashboard_data:
        ...
        metric_data = [
            ...
            ("High/Critical Pairs", metrics.get('high_risk_pairs', 0), "#B45309", "alert"),
            ("Critical Pairs", metrics.get('critical_risk_pairs', 0), COLORS["danger"], "triangle"),
            ("Avg Risk Score", f"{metrics.get('avg_risk_score', 0):.1f}%", COLORS["accent"], "stats")
        ]
    ...
    if st.button("🔄 Refresh Visualizations", type="secondary", use_container_width=True):
        load_dashboard_data()
        st.rerun()
    ...
    <div style='margin-bottom: 2rem;'>
        ...
        <p style='color: {COLORS["gray"]}; margin-bottom: 1.5rem;'>
            Interactive visualization of confusion risk between drug pairs. Hover over cells for detailed risk scores.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if heatmap_chart:
        ...
        <div class='info-box'>
            <p style='margin: 0; display: flex; align-items: flex-start; gap: 15px;'>
                ...
                Light purple cells indicate low risk (<25%), medium purple shows moderate risk (25-50%), 
                dark purple cells indicate high risk (50-75%), and red cells show critical risk (>75%). 
...
with tab3:
    ...
    with col1:
        st.markdown(f"""
        <div class='dashboard-card' style='text-align: left; border-left-color: {COLORS["danger"]};'>
    ...
    with col2:
        st.markdown(f"""
        <div class='dashboard-card' style='text-align: left; border-left-color: {COLORS["success"]};'>
            <div style='display: flex; align-items: center; margin-bottom: 20px;'>
    ...
    for col, (icon, title, subtitle, desc) in zip(steps_cols, step_data):
        with col:
            st.markdown(f"""
            <div class='dashboard-card' style='height: 240px;'>
...
# Disclaimer section
st.markdown("""
<div style='margin-top: 30px; padding-top: 25px; border-top: 1px solid rgba(124, 58, 237, 0.1);'>
...
with st.sidebar:
    ...
    <div style='text-align: center; padding: 2rem 0 1rem 0;'>
        ...
        <h3 style='color: {COLORS["dark"]}; margin: 10px 0 5px 0; font-family: Poppins, sans-serif; font-weight: 800;'>MediNomix</h3>
    ...
    risk_categories = [
        ("Critical", "≥75%", "Immediate intervention required", COLORS["danger"]),
        ("High", "50-74%", "Review and verification needed", "#B45309"),
