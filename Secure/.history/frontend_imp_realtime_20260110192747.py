...
def render_sidebar():
    ...
    with st.sidebar:
        # Enhanced App Header with Modern Gradient
        st.markdown(f"""
        <style>
        @import url('https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css');
        ...
        .sidebar-metric:hover .sidebar-metric-icon {{
            transform: scale(1.2);
        }}
        ...
        .risk-badge.critical {{ background: linear-gradient(135deg, #EF4444 0%, #DC2626 100%); color: white; }}
        .risk-badge.high {{ background: linear-gradient(135deg, #F59E0B 0%, #D97706 100%); color: white; }}
        .risk-badge.medium {{ background: linear-gradient(135deg, #8B5CF6 0%, #7C3AED 100%); color: white; }}
        .risk-badge.low {{ background: linear-gradient(135deg, #10B981 0%, #059669 100%); color: white; }}
        ...
        <div class="sidebar-header">
            ...
            <h2>MediNomix</h2>
            <p>AI-Powered Medication Safety Platform</p>
        </div>
        """, unsafe_allow_html=True)
                ...
                if data.get('status') == 'healthy':
                    ...
                    <div class="sidebar-metric-grid">
                        <div class="sidebar-metric">
                            <div class="sidebar-metric-icon">
                                <i class="fas fa-capsules"></i>
                            </div>
                            <div class="sidebar-metric-label">Drugs</div>
                            <div class="sidebar-metric-value" id="drugs-count">0</div>
                        </div>
                        <div class="sidebar-metric">
                            <div class="sidebar-metric-icon">
                                <i class="fas fa-chart-bar"></i>
                            </div>
                            <div class="sidebar-metric-label">Analyses</div>
                            <div class="sidebar-metric-value" id="analyses-count">0</div>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                    
                    # JavaScript to update metrics
                    st.markdown(f"""
                    <script>
                    document.getElementById('drugs-count').textContent = '{data.get('metrics', {{}}).get('drugs_in_database', 0)}';
                    document.getElementById('analyses-count').textContent = '{data.get('metrics', {{}}).get('total_analyses', 0)}';
                    </script>
                    """, unsafe_allow_html=True)
                else:
        ...
        if st.button("", key="doc_btn", help="Open documentation"):
            render_alert_card("Documentation coming soon!", "info")
        
        st.markdown("""
        <button class="sidebar-button" onclick="alert('Documentation coming soon!')">
            <i class="fas fa-book sidebar-button-icon"></i>
            Documentation
        </button>
        
        <button class="sidebar-button" onclick="alert('Bug reporting coming soon!')">
            <i class="fas fa-bug sidebar-button-icon"></i>
            Report Bug
        </button>
        
        <button class="sidebar-button" onclick="window.location.reload()">
            <i class="fas fa-redo-alt sidebar-button-icon"></i>
            Clear Cache
        </button>
        """, unsafe_allow_html=True)
        ...
        # Risk Categories Guide Card
        st.markdown("""
        <div class="modern-sidebar-card">
            <div class="sidebar-card-header">
                <h3><i class="fas fa-exclamation-triangle"></i> Risk Categories</h3>
            </div>
            <div class="sidebar-card-content">
        """, unsafe_allow_html=True)
        ...
        for name, score, desc, color in risk_levels:
            st.markdown(f"""
            <div class="risk-category-item" style="border-left-color: {color};">
                <div class="risk-category-header">
                    <div class="risk-badge {name.lower()}">{name}</div>
                    <div class="risk-score">{score}</div>
                </div>
                <div class="risk-description">{desc}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        # Footer Note
        st.markdown("""
        <div style="text-align: center; margin-top: 30px; padding: 20px; color: #6B7280; font-size: 11px; font-weight: 500;">
            <div style="display: flex; justify-content: center; gap: 8px; margin-bottom: 8px;">
                <i class="fas fa-shield-alt" style="color: #8B5CF6;"></i>
                <span>Secured Connection</span>
                <i class="fas fa-lock" style="color: #10B981;"></i>
            </div>
            <div>v2.1.0 • Updated Today</div>
        </div>
        """, unsafe_allow_html=True)
# ================================
# FOOTER
