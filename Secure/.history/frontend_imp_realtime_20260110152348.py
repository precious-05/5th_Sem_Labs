...
def create_heatmap_chart():
    """Create interactive drug confusion heatmap with working text annotations"""
    ...
    # Text matrix create karein (only show values > 25%)
    text_matrix = []
    for row in risk_matrix:
        text_row = []
        for val in row:
            if val >= 25:  # Only show values 25% or higher
                text_row.append(f"{val:.0f}%")
            else:
                text_row.append("")
        text_matrix.append(text_row)
    
    # Plotly ka go.Heatmap use karein text ke saath
    fig = go.Figure(data=go.Heatmap(
        z=risk_matrix,
        x=drug_names,
        y=drug_names,
        text=text_matrix,  # Text matrix add karein
        texttemplate="%{text}",  # Text display format
        textfont=dict(
            size=14 if len(drug_names) <= 10 else 11,
            color="white",
            family="Poppins",
            weight="bold"
        ),
        colorscale=[
            [0.0, "#10B981"],   # Green for 0%
            [0.2, "#8B5CF6"],   # Purple for 20%
            [0.4, "#F59E0B"],   # Orange for 40%
            [0.6, "#EF4444"],   # Red for 60%
            [0.8, "#DC2626"],   # Dark red for 80%
            [1.0, "#7F1D1D"]    # Very dark red for 100%
        ],
        zmin=0,
        zmax=100,
        hovertemplate=(
            "<b style='font-size:14px; font-family:Poppins;'>🚨 Confusion Alert</b><br><br>"
            "<b>Drug 1:</b> %{y}<br>"
            "<b>Drug 2:</b> %{x}<br>"
            "<b>Risk Score:</b> <span style='color:#EF4444; font-weight:bold;'>%{z:.1f}%</span><br><br>"
            "<span style='font-size:12px; color:#6B7280;'>Click for detailed analysis</span>"
            "<extra></extra>"
        ),
        colorbar=dict(
            title=dict(
                text="<b>Risk %</b>",
                font=dict(color=COLORS['text_primary'], size=13, family="Poppins")
            ),
            titleside="right",
            tickmode="array",
            tickvals=[0, 25, 50, 75, 100],
            ticktext=["0%", "25%", "50%", "75%", "100%"],
            tickfont=dict(color=COLORS['text_secondary'], size=11, family="Poppins"),
            thickness=20,
            len=0.75,
            xpad=20,
            ypad=10
    ...
    fig.update_layout(
        title=dict(
            ...
            font=dict(color=COLORS['text_primary'], size=22, family="Poppins", weight="bold"),
            x=0.5,
            xanchor="center",
            y=0.95,
            pad=dict(b=20)
        ),
        height=max(500, len(drug_names) * 40),  # Dynamic height based on drugs
        width=max(600, len(drug_names) * 45),   # Dynamic width
        xaxis=dict(
            title=dict(
                text="<b>Drug Names</b>",
                font=dict(color=COLORS['text_primary'], size=14, family="Poppins", weight="bold")
            ),
            tickfont=dict(color=COLORS['text_secondary'], size=12 if len(drug_names) <= 12 else 10, family="Poppins"),
            tickangle=45 if len(drug_names) > 8 else 0,
            side="top",  # Labels top pe
            showgrid=True,
        ...
        yaxis=dict(
            ...
            tickfont=dict(color=COLORS['text_secondary'], size=12 if len(drug_names) <= 12 else 10, family="Poppins"),
            autorange="reversed",  # Standard heatmap format
            showgrid=True,
            gridcolor=COLORS['border'],
            gridwidth=0.5,
            linecolor=COLORS['border'],
        ...
        margin=dict(l=80, r=50, t=120, b=80, pad=10),
        hoverlabel=dict(
            bgcolor="white",
            font_size=13,
            font_family="Poppins",
    ...
    # Add risk level indicators on sides (optional enhancement)
    if len(drug_names) > 0:
        # Highlight diagonal (self-comparison)
        fig.add_shape(
            type="line",
            x0=-0.5, y0=-0.5,
            x1=len(drug_names)-0.5, y1=len(drug_names)-0.5,
