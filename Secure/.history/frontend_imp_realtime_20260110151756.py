...
def create_heatmap_chart():
    """Create interactive drug confusion heatmap with working annotations"""
    ...
    # Create annotations for high-risk cells - FIXED METHOD
    annotations = []
            ...
            if value > 70:  # High risk values
                ...
                annotations.append(
                    dict(
                        ...
                        bgcolor="rgba(0, 0, 0, 0.3)" if value > 80 else "rgba(255, 255, 255, 0.7)",
                        bordercolor=COLORS['danger'] if value > 80 else COLORS['warning'],
                        borderwidth=1,
                        borderpad=3,
                        # CRITICAL FIX: Add these properties
                        xref="x",
                        yref="y"
                    )
                )
    
    # Create heatmap with proper hover information
    fig = go.Figure(data=go.Heatmap(
        ...
        colorscale=[
            ...
            [0.6, COLORS['warning']],
            [0.8, COLORS['danger']],
            [1, '#8B0000']  # Dark red for highest risk
        ],
        zmin=0,
        zmax=100,
        hovertemplate=(
            "<b>Drug 1: %{y}</b><br>"
            "<b>Drug 2: %{x}</b><br>"
            "<b>Confusion Risk: %{z:.1f}%</b><br>"
            "<extra></extra>"
        ),
        colorbar=dict(
            title=dict(
                text="Risk %",
                font=dict(color=COLORS['text_primary'], size=12, family="Poppins")
            ),
            titleside="right",
        ...
        # Add cell outlines for better visibility
        xgap=1,
        ygap=1
    ))
    
    # Update layout with annotations - FIXED
    fig.update_layout(
        title=dict(
            text="Drug Confusion Risk Heatmap",
            font=dict(color=COLORS['text_primary'], size=20, family="Poppins", weight="bold"),
            x=0.5,
            xanchor="center",
            y=0.95
        ),
        ...
        xaxis=dict(
            title="Drug Names",
            titlefont=dict(color=COLORS['text_primary'], size=14, family="Poppins"),
            tickfont=dict(color=COLORS['text_secondary'], size=11, family="Poppins"),
            gridcolor=COLORS['border'],
            linecolor=COLORS['border'],
            linewidth=1,
            tickangle=45 if len(drug_names) > 10 else 0,
            side="top"  # X-axis labels top pe
        ),
        yaxis=dict(
            title="Drug Names",
            titlefont=dict(color=COLORS['text_primary'], size=14, family="Poppins"),
            ...
            linecolor=COLORS['border'],
            linewidth=1,
            autorange="reversed"  # Heatmap ka standard layout
        ),
        plot_bgcolor='white',
        paper_bgcolor='white',
        font_color=COLORS['text_primary'],
        margin=dict(l=80, r=50, t=100, b=80),
        # CRITICAL: Add annotations here
        annotations=annotations if annotations else []
    )
    
    # Add shapes for high-risk cells (alternative to annotations)
    shapes = []
    for i in range(len(drug_names)):
        for j in range(len(drug_names)):
            value = risk_matrix[i][j]
