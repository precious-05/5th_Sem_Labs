...
def render_analytics_tab():
    ...
    if 'metrics' in st.session_state.dashboard_data:
        ...
        with col1:
            st.markdown(f"""
            <div class="kpi-card">
                <div class="kpi-icon" style="background: rgba(16, 185, 129, 0.1); border: 2px solid rgba(16, 185, 129, 0.2);">
                    <i class="fas fa-pills" style="color: #10B981;"></i>
                </div>
                <div class="kpi-value">{metrics.get('total_drugs', 0)}</div>
        ...
        with col2:
            st.markdown(f"""
            <div class="kpi-card">
                <div class="kpi-icon" style="background: rgba(239, 68, 68, 0.1); border: 2px solid rgba(239, 68, 68, 0.2);">
                    <i class="fas fa-fire" style="color: #EF4444;"></i>
                </div>
                <div class="kpi-value">{metrics.get('critical_risk_pairs', 0)}</div>
                <div class="kpi-label">Critical Pairs</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col3:
            st.markdown(f"""
            <div class="kpi-card">
                <div class="kpi-icon" style="background: rgba(245, 158, 11, 0.1); border: 2px solid rgba(245, 158, 11, 0.2);">
                    <i class="fas fa-exclamation-triangle" style="color: #F59E0B;"></i>
                </div>
                <div class="kpi-value">{metrics.get('high_risk_pairs', 0)}</div>
        ...
        with col4:
            st.markdown(f"""
            <div class="kpi-card">
                <div class="kpi-icon" style="background: rgba(139, 92, 246, 0.1); border: 2px solid rgba(139, 92, 246, 0.2);">
                    <i class="fas fa-chart-line" style="color: #8B5CF6;"></i>
                </div>
                <div class="kpi-value">{metrics.get('avg_risk_score', 0):.1f}%</div>
    ...
    st.markdown(f"""
    <div class="analytics-charts-card">
        <div class="charts-header">
            <div class="charts-icon">
                <i class="fas fa-chart-pie"></i>
            </div>
            <div>
                <h2 class="charts-title">Analytics Charts</h2>
                <p class="charts-subtitle">Visual insights into medication confusion risks</p>
            </div>
        </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        <div class="chart-card">
            <div class="chart-card-header">
                <i class="fas fa-chart-pie chart-card-icon" style="color: #8B5CF6;"></i>
    ...
    with col2:
        ...
        <div class="chart-card">
            <div class="chart-card-header">
                <i class="fas fa-chart-bar chart-card-icon" style="color: #EC4899;"></i>
                <h3 class="chart-card-title">Top 10 High-Risk Drug Pairs</h3>
            </div>
        """, unsafe_allow_html=True)
    ...
    <div class="heatmap-card">
        <div class="heatmap-header">
            ...
            <div>
                <h2 class="heatmap-title">Drug Confusion Risk Heatmap</h2>
                <p class="heatmap-subtitle">Visual matrix of potential drug confusion risks</p>
            </div>
        </div>
    """, unsafe_allow_html=True)
    ...
    # FDA Alerts Card
    st.markdown(f"""
    <div class="fda-alerts-card">
        <div class="fda-header">
            <div class="fda-icon">
                <i class="fas fa-exclamation-circle"></i>
            </div>
            <div>
                <h2 class="fda-title">FDA High Alert Drug Pairs</h2>
                <p class="fda-subtitle">Most commonly confused drug pairs according to FDA</p>
            </div>
        </div>
    """, unsafe_allow_html=True)
    ...
    st.dataframe(
        ...
        hide_index=True,
        column_config={
            "Drug 1": st.column_config.TextColumn(
                "Drug 1",
                help="First drug in the pair",
                width="medium"
            ),
            "Drug 2": st.column_config.TextColumn(
                "Drug 2",
                help="Second drug in the pair",
                width="medium"
            ),
            ...
            "Reason": st.column_config.TextColumn(
                "Reason",
                help="Potential confusion reason",
                width="large"
            )
        }
    )
    ...
    # Global CSS Styles for Analytics Tab
    st.markdown("""
    <style>
    .kpi-grid {
        display: grid;
    ...
    .analytics-charts-card, .heatmap-card, .fda-alerts-card {
        background: linear-gradient(135deg, #FFFFFF 0%, #F9F7FF 100%);
        padding: 32px;
        border-radius: 24px;
        border: 2px solid #E5E7EB;
    ...
    .analytics-charts-card:hover, .heatmap-card:hover, .fda-alerts-card:hover {
        border-color: #D6BCFA;
        box-shadow: 0 12px 40px rgba(139, 92, 246, 0.15);
    }
    
    .charts-header, .heatmap-header, .fda-header {
        display: flex;
        align-items: center;
        gap: 20px;
        margin-bottom: 32px;
    ...
    .charts-icon, .heatmap-icon, .fda-icon {
        font-size: 36px;
        color: #7C3AED;
        background: rgba(139, 92, 246, 0.1);
        width: 64px;
    ...
    .charts-title, .heatmap-title, .fda-title {
        margin: 0;
        color: #1F2937;
        font-weight: 800;
        font-size: 24px;
    ...
    .charts-subtitle, .heatmap-subtitle, .fda-subtitle {
        color: #6B7280;
        margin: 6px 0 0 0;
        font-size: 14px;
        font-weight: 500;
    ...
    .dataframe th {
        background: linear-gradient(135deg, #F5F3FF 0%, #F0EDFF 100%) !important;
        color: #1F2937 !important;
        font-weight: 600 !important;
        border: none !important;
    }
