...
def create_heatmap_chart():
    """Create heatmap with automatic text color based on background"""
    ...
    # Create figure with Plotly Express (text_auto works better)
    import plotly.express as px
    
    # Create DataFrame
    df = pd.DataFrame(risk_matrix, index=drug_names, columns=drug_names)
    ...
    fig = px.imshow(
        ...
        range_color=[0, 100],
        labels=dict(x="Drug 2", y="Drug 1", color="Risk %"),
        text_auto=".0f",  # Automatically shows values
    )
    ...
    fig.update_traces(
        ...
        textfont=dict(
            size=13,
            color="white",  # White text on all cells
            family="Poppins",
            weight="bold"
        ),
        hovertemplate=(
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            title="Drug Names",
            title_font=dict(size=14, family="Poppins", color=COLORS['text_primary']),
            tickfont=dict(size=11, family="Poppins", color=COLORS['text_secondary']),
        ...
        coloraxis_colorbar=dict(
            title="Risk %",
            title_font=dict(size=12, family="Poppins", color=COLORS['text_primary']),
            tickfont=dict(size=10, family="Poppins", color=COLORS['text_secondary']),
            thickness=20,
            len=0.75
        ),
        plot_bgcolor='white',
