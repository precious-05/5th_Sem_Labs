...
def create_heatmap_chart():
    """Create interactive drug confusion heatmap with annotations - FIXED VERSION"""
    ...
    # ALTERNATIVE ANNOTATION METHOD - Using text overlay
    annotation_text = []
    for i in range(len(risk_matrix)):
        ...
        for j in range(len(risk_matrix[i])):
            ...
            if value > 70:
                row_text.append(f'🔥<br><b>{value:.0f}%</b>')
            elif value > 50:
                row_text.append(f'⚠️<br><b>{value:.0f}%</b>')
            else:
                row_text.append(f'{value:.0f}%')
        annotation_text.append(row_text)
    
    # Modern purple gradient colorscale
    colorscale = [
        [0.0, '#F5F3FF'],      # Very light purple
        [0.25, '#DDD6FE'],     # Light purple
        [0.5, '#8B5CF6'],      # Primary purple
        [0.75, '#7C3AED'],     # Dark purple
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        zmin=0,
        zmax=100,
        text=annotation_text,  # ✅ TEXT OVERLAY
        texttemplate='%{text}',
        textfont=dict(
            color=['white' if val > 50 else '#374151' for row in risk_matrix for val in row],
            size=10,
            family="Inter"
        ),
        hovertemplate=(
        ...
        colorbar=dict(
            ...
            ticktext=["0% (Safe)", "25%", "50%", "75%", "100% (Critical)"],
            len=0.8,
            thickness=15,
            x=1.02
        ),
