...
def render_sidebar():
    ...
    with st.sidebar:
            ...
            if response.status_code == 200:
                ...
                if data.get('status') == 'healthy':
                    ...
                    with col1:
                        ...
                        """.format(data.get('metrics', {}).get('drugs_in_database', 0)), unsafe_allow_html=True)
                    with col2:
                        st.markdown("""
                        <div class="sidebar-metric">
                        ...
                        """.format(data.get('metrics', {}).get('total_analyses', 0)), unsafe_allow_html=True)
                else:
        ...
        if st.button("", key="docs_btn", use_container_width=True):
        ...
        if st.button("", key="bug_btn", use_container_width=True):
        ...
        if st.button("", key="cache_btn", use_container_width=True):
            st.session_state.search_results = []
            st.session_state.dashboard_data = {}
        ...
        for name, score, desc, color in risk_levels:
            ...
            <div class="risk-category-item">
                <div class="risk-header">
                    <div class="risk-badge" style="background: {color};">{name}</div>
        ...
        /* Sidebar Metrics */
        .sidebar-metric {{
            background: linear-gradient(135deg, rgba(245, 243, 255, 0.8) 0%, rgba(240, 249, 255, 0.8) 100%);
            padding: 16px;
            border-radius: 12px;
            text-align: center;
