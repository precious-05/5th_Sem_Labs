...
def create_heatmap_chart():
    """Create professional drug confusion heatmap with annotations"""
    ...
    # Create text annotations (only show high risk values)
    text_matrix = []
    color_matrix = []  # For cell background colors
    
    for row in risk_matrix:
        text_row = []
        color_row = []
        for val in row:
            if val >= 70:  # Critical risk
                text_row.append(f"<b>{int(val)}%</b>")
                color_row.append('white')  # White text on dark background
            elif val >= 50:  # High risk
                text_row.append(f"{int(val)}%")
                color_row.append('white')
            elif val >= 30:  # Medium risk
                text_row.append(f"{int(val)}%")
    ...
    colorscale = [
        ...
        [0.3, "#34D399"],     # Light Green
        [0.5, "#F59E0B"],     # Orange - Medium risk
        [0.7, "#EF4444"],     # Red - High risk
        [0.9, "#DC2626"],     # Dark Red
        [1.0, "#7F1D1D"]      # Very Dark Red - Critical risk
    ]
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        hoverinfo="z+text",
        hovertemplate=(
            "<b style='font-family:Poppins; font-size:14px;'>💊 Drug Confusion Alert</b><br><br>"
            "<b>Drug 1:</b> %{y}<br>"
            "<b>Drug 2:</b> %{x}<br>"
            "<b>Risk Score:</b> <span style='color:#EF4444; font-weight:bold;'>%{z:.1f}%</span><br>"
        ...
        colorbar=dict(
            ...
            tickfont=dict(color=COLORS['text_secondary'], size=12, family="Poppins"),
            thickness=25,
            len=0.8,
            x=1.02,
            xpad=30,
        ...
        # Add cell spacing and borders
        xgap=2,
        ygap=2,
        showscale=True,
        zsmooth=False,  # Keep cells sharp
        connectgaps=False
    ))
    
    # Add borders to critical cells
    shapes = []
    for i in range(len(drug_names)):
        for j in range(len(drug_names)):
            value = risk_matrix[i][j]
            if value > 80:  # Critical risk cells
                shapes.append({
                    'type': 'rect',
                    'xref': 'x',
                    'yref': 'y',
    ...
    fig.update_layout(
        ...
        height=max(600, len(drug_names) * 45),
        width=max(700, len(drug_names) * 50),
        xaxis=dict(
            title=dict(
                text="<b>Drug Names</b>",
                font=dict(
            ...
            tickfont=dict(
                ...
                size=12 if len(drug_names) <= 12 else 10,
                family="Poppins"
            ),
        ...
        margin=dict(l=100, r=150, t=150, b=100, pad=15),
        hoverlabel=dict(
            bgcolor="white",
            font_size=14,
            font_family="Poppins",
    ...
    # Add diagonal line for self-comparison
    fig.add_shape(
        type="line",
        x0=-0.5, y0=-0.5,
