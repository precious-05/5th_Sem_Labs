...
def create_heatmap_chart():
    """Create premium interactive drug confusion heatmap with medical dark theme"""
    ...
    if not drug_names or not risk_matrix:
        # Return a beautiful placeholder
        return create_heatmap_placeholder()
    
    # Premium color palette for medical dark theme
    MEDICAL_COLORS = {
        'bg_dark': '#0A0E17',
        'bg_card': 'rgba(18, 23, 36, 0.85)',
        'grid_lines': 'rgba(59, 130, 246, 0.15)',
        'text_primary': '#FFFFFF',
        ...
        'accent_cyan': '#06B6D4',
        'gradient_medical': 'linear-gradient(135deg, #10B981 0%, #3B82F6 50%, #8B5CF6 100%)',
    }
    
    # Create premium annotations with glow effect
    annotations = []
    glow_annotations = []
    
    for i, row in enumerate(risk_matrix):
        for j, value in enumerate(row):
            if value > 75:  # Critical risk
                # Glow effect annotation (behind)
                glow_annotations.append(
                    dict(
                        x=j,
                        y=i,
                        text="🔥",
                        showarrow=False,
                        font=dict(color=MEDICAL_COLORS['accent_danger'], size=28, family="Poppins"),
                        opacity=0.3,
                        xanchor="center",
                        yanchor="middle"
                    )
                ...
                annotations.append(
                    dict(
                        ...
                        bgcolor=f"rgba(239, 68, 68, {0.9 + (value-75)/25 * 0.1})",
                ...
                annotations.append(
                    dict(
                        ...
                        text=f"<b>{value:.0f}%</b>",
                        showarrow=False,
                        font=dict(color=MEDICAL_COLORS['text_primary'], size=10, family="Poppins"),
                        bgcolor=f"rgba(245, 158, 11, {0.7 + (value-60)/15 * 0.2})",
    ...
    # Create the heatmap with premium styling
    fig = go.Figure(data=go.Heatmap(
        z=risk_matrix,
        x=drug_names,
        y=drug_names,
        colorscale=[
            [0.0, 'rgba(16, 185, 129, 0.1)'],      # Low risk - transparent green
            [0.25, 'rgba(139, 92, 246, 0.3)'],      # Medium low - purple
            [0.5, 'rgba(245, 158, 11, 0.5)'],       # Medium - amber
            [0.75, 'rgba(239, 68, 68, 0.7)'],       # High - red
            [1.0, 'rgba(220, 38, 38, 0.9)']         # Critical - dark red
        ],
        zmin=0,
        zmax=100,
        hoverongaps=False,
        hoverinfo="z+x+y",
        hovertemplate="""
        <div style='background: rgba(18, 23, 36, 0.95); padding: 12px; border-radius: 8px; border: 1px solid rgba(59, 130, 246, 0.3);'>
            <span style='color: #10B981; font-size: 11px;'>DRUG PAIR</span><br>
            <div style='display: flex; align-items: center; gap: 8px; margin: 8px 0;'>
                <div style='background: rgba(59, 130, 246, 0.2); padding: 4px 12px; border-radius: 20px; font-weight: 700; color: #3B82F6;'>%{y}</div>
                <div style='color: #94A3B8;'>↔</div>
                <div style='background: rgba(139, 92, 246, 0.2); padding: 4px 12px; border-radius: 20px; font-weight: 700; color: #8B5CF6;'>%{x}</div>
            </div>
            <div style='margin-top: 12px;'>
                <span style='color: #EF4444; font-size: 11px;'>CONFUSION RISK</span><br>
                <div style='font-size: 24px; font-weight: 800; color: %{z > 75 ? '#EF4444' : '%{z > 50 ? '#F59E0B' : '#10B981'}'}; margin: 4px 0;'>
                    %{z:.1f}%
                </div>
            </div>
            <div style='margin-top: 8px; font-size: 10px; color: #64748B;'>
                <span style='color: #94A3B8;'>Click for detailed analysis</span>
            </div>
        </div>
        <extra></extra>
        """,
        colorbar=dict(
            ...
            ticktext=["SAFE", "LOW", "MED", "HIGH", "CRITICAL"],
            tickfont=dict(
            ...
            tickcolor=MEDICAL_COLORS['grid_lines'],
            ticklen=10,
            tickwidth=2,
            thickness=20,
            len=0.8,
    ...
    fig.update_layout(
        title=dict(
            text="💊 MEDICATION CONFUSION RISK MATRIX",
            font=dict(
                color=MEDICAL_COLORS['text_primary'], 
                size=20, 
                family="Poppins",
        ...
        plot_bgcolor=MEDICAL_COLORS['bg_card'],
        paper_bgcolor='rgba(0,0,0,0)',  # Fully transparent
        font_color=MEDICAL_COLORS['text_primary'],
        margin=dict(l=100, r=120, t=80, b=100, pad=10),
        
        # X-axis styling
        xaxis=dict(
            title=dict(
                text="<b>TARGET MEDICATIONS</b>",
                font=dict(
                    color=MEDICAL_COLORS['text_secondary'], 
                    size=11, 
                    family="Poppins",
            ...
            tickfont=dict(
                color=MEDICAL_COLORS['text_muted'], 
                size=10, 
                family="Poppins",
                weight="500"
            ),
            gridcolor=MEDICAL_COLORS['grid_lines'],
            gridwidth=0.5,
            linecolor=MEDICAL_COLORS['grid_lines'],
            linewidth=1,
            mirror=True,
            showline=True,
            ticks="outside",
        ...
        # Add annotations (glow first, then text)
        annotations=glow_annotations + annotations,
        
        # Add shapes for highlight
        shapes=[
            ...
            dict(
                ...
                y1=1,
                line=dict(
                    color=MEDICAL_COLORS['grid_lines'],
                    width=1,
                ),
        ...
        # Modebar styling
        modebar=dict(
            bgcolor='rgba(18, 23, 36, 0.8)',
            color=MEDICAL_COLORS['text_secondary'],
            activecolor=MEDICAL_COLORS['accent_primary']
        )
    )
    ...
    if len(drug_names) > 1:
        fig.add_shape(
            ...
            x0=-0.5,
            y0=-0.5,
            x1=len(drug_names)-0.5,
            y1=len(drug_names)-0.5,
            line=dict(
                color=MEDICAL_COLORS['grid_lines'],
                width=1,
                dash="dash",
...
def create_heatmap_placeholder():
    """Create a beautiful placeholder when no data is available"""
    MEDICAL_COLORS = {
        'bg_dark': '#0A0E17',
        'bg_card': 'rgba(18, 23, 36, 0.85)',
        'text_primary': '#FFFFFF',
    ...
    fig.update_layout(
        ...
        annotations=[
            dict(
                text="<span style='font-size:48px;'>📊</span><br><br>"
                     "<span style='color: #94A3B8; font-size: 16px;'>Heatmap data will appear here</span><br>"
                     "<span style='color: #64748B; font-size: 12px;'>Search for medications first to generate the risk matrix</span>",
                xref="paper",
                yref="paper",
                x=0.5,
                y=0.5,
                showarrow=False,
                font=dict(size=14, color=MEDICAL_COLORS['text_secondary']),
                align="center",
                bgcolor="rgba(18, 23, 36, 0.5)",
                bordercolor=MEDICAL_COLORS['accent_primary'],
                borderwidth=1,
                borderpad=20,
                opacity=0.8
            )
