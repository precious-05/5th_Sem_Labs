...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        <div style="text-align: center; margin-bottom: 24px;">
            <div class="medinomia-header-card">
                <div class="header-card-icon">
                    <i class="fas fa-stethoscope"></i>
                </div>
                <h2 class="header-card-title">MediNomix</h2>
                <p class="header-card-subtitle">AI Medication Safety</p>
            </div>
        </div>
        
        <style>
        .medinomia-header-card {{
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('status') == 'healthy':
                    ...
                    render_alert_card("Backend server has issues", "warning", "Backend Issues")
            else:
        ...
        if st.button("🐞 Report Bug", key="bug_btn", use_container_width=True):
            render_alert_card("Bug reporting coming soon!", "info")
        
        if st.button("🗑️ Clear Cache", key="cache_btn", use_container_width=True):
            st.session_state.search_results = []
            st.session_state.dashboard_data = {}
        ...
        st.markdown("""
        <div class="sidebar-risk-card">
            <div class="risk-header">
                <i class="fas fa-exclamation-triangle risk-icon"></i>
                <span class="risk-title">Risk Categories</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        for name, score, desc, color in risk_levels:
            ...
            <div class="risk-item-card">
                <div class="risk-item-header">
                    <div class="risk-badge" style="background: {color}; color: white;">{name}</div>
        ...
        <style>
        /* Sidebar Cards */
        .sidebar-status-card, .sidebar-links-card, .sidebar-risk-card {{
            background: linear-gradient(135deg, rgba(248, 247, 255, 0.95) 0%, rgba(243, 242, 255, 0.95) 100%);
            padding: 20px;
            border-radius: 16px;
            margin-bottom: 20px;
        ...
        .status-header, .links-header, .risk-header {{
            display: flex;
            align-items: center;
            gap: 12px;
            margin-bottom: 16px;
        ...
        .status-icon, .links-icon, .risk-icon {{
            color: #8B5CF6;
            font-size: 18px;
        }}
        
        .status-title, .links-title, .risk-title {{
            color: #1F2937;
            font-size: 16px;
            font-weight: 700;
            margin: 0;
        ...
        /* Sidebar Metrics */
        .sidebar-metric-card {{
            background: linear-gradient(135deg, rgba(245, 243, 255, 0.9) 0%, rgba(240, 249, 255, 0.9) 100%);
            padding: 16px;
            border-radius: 12px;
            text-align: center;
