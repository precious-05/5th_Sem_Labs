...
(function() {
    ...
    function showDetails(index) {
        if (filteredHistory[index]) {
            ...
            alert(`Details for Patient ${index + 1}\nRisk: ${item.risk_percentage}%\nPrediction: ${item.prediction}`);
        }
    }
    ...
    // Add this function to your history.js file
function showDetails(index) {
    console.log("View details for index:", index);
    
    // Get the item from filtered history
    if (filteredHistory[index]) {
        const item = filteredHistory[index];
        
        // Create a simple modal or show alert
        const modalContent = `
            <h3>Patient Details</h3>
            <p><strong>Date:</strong> ${new Date(item.timestamp).toLocaleString()}</p>
            <p><strong>Risk Score:</strong> ${item.risk_percentage}%</p>
            <p><strong>Prediction:</strong> ${item.prediction}</p>
            <p><strong>Age:</strong> ${item.user_data?.Age || 'N/A'}</p>
            <p><strong>Gender:</strong> ${item.user_data?.Gender_Male === 1 ? 'Male' : 'Female'}</p>
        `;
        
        // Show alert (temporary solution)
        alert(modalContent);
        
        // Or show in a modal if you have one
