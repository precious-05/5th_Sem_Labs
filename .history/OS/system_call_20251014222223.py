...
# 1 Open the file using OS-level open (not the built-in open())
# Flags: os.O_RDONLY means "open as read-only"
fd = os.open("/etc/hosts", os.O_RDONLY)

try:
    while True:
        #  Read up to 256 bytes from file descriptor
        data = os.read(fd, 256)
        ...
        #  Write the data to standard output (same as write(STDOUT_FILENO, ...))
